...
- TASK-007 已让人工确认后的正式 MP4 默认烧录固定轻量样式字幕，同时继续生成同名独立
  SRT，并修复 Windows 启动器的 Python 选择和资产根提示。
...
5. 预览候选，调整入出点，明确勾选人工确认，再导出一个带画面字幕的 MP4 和同名独立 SRT。

启动器按顺序选择仓库 `.venv`、`LIVECLIP_ASSETS_ROOT` 下的 `.venv` 或 PATH 中第一个真实
Python，并自动排除 WindowsApps 应用执行别名；不需要关闭该别名。未配置资产根且仓库内
资产不完整时，错误信息会列出缺失项并给出可复制的持久设置示例：

```powershell
[Environment]::SetEnvironmentVariable("LIVECLIP_ASSETS_ROOT", "<包含模型、FFmpeg和ASR环境的目录>", "User")
```

本地资产根需包含下文列出的 FFmpeg、ASR 运行环境和模型；文本分析使用以下三个进程环境变量：
...
导出成功后页面显示 MP4/SRT 文件名、最终时间范围、实际时长、输出文件夹名称，以及
“视频字幕：已烧录、独立字幕：已生成”，不显示绝对路径。

未提供 `--output` 时，MP4 和 SRT 写入原视频旁的 `<视频名>_exports`。输出采用 H.264 +
AAC；同一份从 timeline 按最终范围裁剪、平移到 0 的 SRT 会写为独立文件并交给 FFmpeg
烧录。字幕固定为微软雅黑优先、白字、黑色描边、底部居中并保留安全边距，按视频高度只取
三档字号；不提供字体、颜色、位置、动画或字幕编辑器。成功后在 analysis 同级目录原子写入
含 `subtitles_burned_in=true` 的 `review_current.json`。既有同名输出不会被覆盖，FFmpeg 或
ffprobe 失败不会发布伪完成 MP4/review，原视频、timeline 和 analysis 不会被修改。视频、
字幕和审核数据不会由审核页面发送到互联网，页面也不加载第三方资源。

预览只支持当前浏览器能够直接解码的 MP4，本版本不生成代理。当前也不支持批量或多片段
导出、拼接、波形、缩略图、字幕样式编辑、翻译/纠错/改写、竖屏转换、队列、自动发布、
安装包或 TASK-008。
